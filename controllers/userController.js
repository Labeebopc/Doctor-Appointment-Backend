...
const Appointment = require("../models/appointmentModal.js");
...
exports.getAllNotification = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ status: true, notification: adminUser.notification });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ status: false, message: error.message });
  }
...
exports.getAllDoctors = asyncHandler(async (req, res) => {
  try {
    const allDoctors = await Doctor.find({ confirmation: "Accepted" }).select(
      "-password"
    );
...
//@disc Book New Appointment
//@api POST /book_new_appointment
//@access Private
exports.bookNewAppointment = asyncHandler(async (req, res) => {
  try {
